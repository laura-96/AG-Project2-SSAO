...
TexturingGLWidget::TexturingGLWidget(QWidget *parent) : QOpenGLWidget(parent)
{
...
TexturingGLWidget::~TexturingGLWidget()
{
    cleanup();
}

QSize TexturingGLWidget::minimumSizeHint() const
{
    return QSize(50, 50);
}

QSize TexturingGLWidget::sizeHint() const
{
    return QSize(m_width, m_height);
}

void TexturingGLWidget::cleanup()
{
	makeCurrent();
...
void TexturingGLWidget::initializeGL()
{
    ...
    connect(context(), &QOpenGLContext::aboutToBeDestroyed, this, &TexturingGLWidget::cleanup);
...
void TexturingGLWidget::paintGL()
{
	// Paint the scene
...
void TexturingGLWidget::resizeGL(int w, int h)
{
	m_width = w;
	m_height = h;
...
void TexturingGLWidget::keyPressEvent(QKeyEvent *event)
{
	switch (event->key()) {
		case Qt::Key_B:
...
void TexturingGLWidget::mousePressEvent(QMouseEvent *event)
{
	m_xClick = event->x();
	m_yClick = event->y();
...
void TexturingGLWidget::mouseMoveEvent(QMouseEvent *event)
{
	makeCurrent();
...
void TexturingGLWidget::mouseReleaseEvent(QMouseEvent *event)
{
	m_doingInteractive = NONE;
	event->ignore();
}

void TexturingGLWidget::wheelEvent(QWheelEvent* event)
{
...
void TexturingGLWidget::loadShaders()
{
	...
	vs.compileSourceFile("./shaders/texturing.vert");
	fs.compileSourceFile("./shaders/texturing.frag");
	...
	m_tex1TextureLoc = glGetUniformLocation(m_program->programId(), "tex1Texture");
	m_tex2LoadedLoc = glGetUniformLocation(m_program->programId(), "tex2Loaded");
	m_tex2TextureLoc = glGetUniformLocation(m_program->programId(), "tex2Texture");
...
void TexturingGLWidget::projectionTransform()
{
	// Set the camera type
	glm::mat4 proj(1.0f);
...
void TexturingGLWidget::resetCamera()
{
	makeCurrent();
	m_radsZoom = 0.0f;
	m_xPan = 0.0f;
...
void TexturingGLWidget::viewTransform()
{
...
void TexturingGLWidget::changeBackgroundColor() {
	
	m_bkgColor = QColorDialog::getColor();
	repaint();
}

void TexturingGLWidget::createBuffersScene()
{
...
void TexturingGLWidget::computeBBoxScene()
{
...
void TexturingGLWidget::sceneTransform()
{
...
void TexturingGLWidget::loadTex1Texture(QString filename)
{
	makeCurrent();
...
void TexturingGLWidget::deleteTex1Texture()
{
	if (m_tex1Tex != nullptr)
	{
		makeCurrent();
...
void TexturingGLWidget::loadTex2Texture(QString filename)
{
	makeCurrent();
...
void TexturingGLWidget::deleteTex2Texture()
{
	if (m_tex2Tex != nullptr)
	{
		makeCurrent();
...
void TexturingGLWidget::setLighting()
{
